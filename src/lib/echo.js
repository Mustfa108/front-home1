...
const DEFAULT_REVERB_HOST = "reverb.sci-syria.org";
...
export function createEcho(actor = "user") {
  ...
  const tokenKey =
    actor === "admin" ? "humascale_admin_token" : "humascale_user_token";
  ...
  const scheme = (
    import.meta.env.VITE_REVERB_SCHEME || DEFAULT_REVERB_SCHEME
  ).toLowerCase();
  const forceTLS = scheme === "https";
  const wsHost = import.meta.env.VITE_REVERB_HOST || DEFAULT_REVERB_HOST;
  const wsPort = Number(
    import.meta.env.VITE_REVERB_PORT || (forceTLS ? 443 : DEFAULT_REVERB_PORT),
  );
