...
export default function ProjectReview() {
  ...
  const handleSubmit = async (event) => {
    ...
    setCreating(true);
    try {
      const response = await projectReviewsApi.create({
        ...form,
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        subtitle="حوّل وصف مشروعك الحقيقي إلى قراءة عملية، مخاطر قابلة للتحقق، وخطوات تنفيذ."
        actions={
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(330px,0.75fr)]">
        ...
        <div className="space-y-6">
          <Card className="overflow-hidden border-brand-200 bg-gradient-to-br from-brand-700 via-brand-600 to-indigo-700 text-white">
            <CardBody>
              ...
              <p className="mt-4 text-sm leading-7 text-brand-100">
                بعد التحليل، اسأل المساعد عن أي توصية أو خطر. سيجيب من واقع نتيجة مشروعك، لا من نصائح عامة منفصلة عن السياق.
              </p>
              <div className="mt-5 grid grid-cols-2 gap-2 text-xs text-brand-50">
                <div className="rounded-xl bg-white/10 p-3">نقاط القوة</div>
                <div className="rounded-xl bg-white/10 p-3">المخاطر</div>
                <div className="rounded-xl bg-white/10 p-3">خطوات تنفيذ</div>
                <div className="rounded-xl bg-white/10 p-3">مؤشرات KPI</div>
              </div>
            </CardBody>
          </Card>
  ...
  return (
    <section className="mt-6 space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card><CardBody><p className="text-sm text-slate-500">مرحلة التنفيذ</p><p className="mt-3 text-xl font-bold text-slate-900">{stageLabels[review.stage] || review.stage}</p><span className="mt-2 inline-flex items-center gap-1 text-xs text-slate-500"><CheckCircle2 size={14} /> بيانات محفوظة للمراجعة</span></CardBody></Card>
      </div>
